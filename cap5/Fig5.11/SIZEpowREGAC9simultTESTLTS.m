...
StatSIZEsimult=zeros(numel(sizevect),7);
StatSIZEindiv=zeros(numel(sizevect),7);
...
for nind=1:6
    ...
    parfor j=1:nsimul
        ...
        SizematLTS(j,:)=[res' sizeind sizesim medres2 outLXS.scale^2];
         ...
         %% 3.LTS with concentration steps bdp=0.5
        outLXScs=LXS(ysim,X,'nsamp',nsamp,'lms',2,'msg',0,'bdp',0.5);
        [res,sizeind,sizesim,medres2]=ResandSize(ysim,XX,outLXS.beta,outLXS.scale^2,thresh,threshBonf);

        % STORING FOR LTS
        SizematLTScs(j,:)=[res' sizeind sizesim medres2 outLXS.scale^2];
    ...
    SIZ(6,:)=mean(SizematLTScs(:,n+1:end));

    % store the statistics
    StatSIZEsimult(ij,1:7)=[sizevect(ij) SIZ(1,1) SIZ(2,1) SIZ(3,1) SIZ(4,1) SIZ(5,1) SIZ(6,1)];
    StatSIZEindiv(ij,1:7)=[sizevect(ij) SIZ(1,2) SIZ(2,2) SIZ(3,2) SIZ(4,2) SIZ(5,2) SIZ(6,2)];
...
% LTScs black
set(plot1(6),'LineStyle','-','Color',[0 0 0]);
...
text(6,StatSIZEsimult(5,7),'LTScs','FontSize',fsize)

%legend('S','MM', 'LTS', 'LTSr', 'FS','FontSize',16, 'Location','southeast')
legend('S','MM', 'LTS', 'LTSr', 'FS','LTScs','FontSize',fsize, 'Location','northeast')
...
%% save part
savestring = ['save size_nsimul_newsim2' num2str(nsimul) 'n' num2str(n) 'p' num2str(p) '.mat, StatSIZEsimult'];
