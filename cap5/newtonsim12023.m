...
% else only the plot
est=1;
% con est=0 genera Figure 5.13 e 5.14
...
for la=lam
    ...
    for j=1:nsimul
        ...
        if est==1
            ...
            %             [outold]=FSRold(ysimj,Xsimj,'nsamp',nsamp,'plots',0,'msg',0,'init',n/2);
            %
            %             if ~isequal(out.ListOut,outold.ListOut) && max(isnan(out.ListOut))+max(isnan(outold.ListOut))<2
            %                 disp([j  length(out.ListOut) length(outold.ListOut)])
            %
            %                 if out.tmptmp==1
            %                     disp('new')
            %                     disp([out.ListOut])
   ...
   % disp(['Simulations finished for  la=' num2str(la)])
    ...
    if ~isempty(intersect(sel,ij))
        ijl=ijl+1;
        %  xl1=min(min((Xysto(:,1,sel))));
        % xl2=max(max((Xysto(:,1,sel))));
        %
        %
        % yl1=min(min((Xysto(:,2,sel))));
        % yl2=max(max((Xysto(:,2,sel))));
        xl1=-20;
        xl2=40;
        yl1=-50;
        yl2=85;
        ...
        if p==1
            ...
            plot(Xsimj(n1+1:end,1),ysimj(n1+1:end),'r+','MarkerSize',8)
            ...
            kcolor='blue';
            lwd=2;
            % line which passes in the middle of the parallelogram
            plot([aunif;aunif],[yxminya;yxmaxya],'color',kcolor,'LineWidth',lwd)
            
            
            yxminyb=bunif*beta+alpha-2*sigmay;
            yxmaxyb=bunif*beta+alpha+2*sigmay;
            
            % vertical line on the right of the parallelogram
            plot([bunif;bunif],[yxminyb;yxmaxyb],'color',kcolor,'LineWidth',lwd)
            
            % vertical line on the left of the parallelogram
            plot([aunif;bunif],[yxminya;yxminyb],'color',kcolor,'LineWidth',lwd)
            % line on top and at the bottom of the parallelogram
            plot([aunif;bunif],[yxmaxya;yxmaxyb],'color',kcolor,'LineWidth',lwd)
            ...
            if prin==1
                % print to postscript
                print -depsc figs\sim1scarev1.eps;
            end
        else
    ...
    if est==1
        ...
        empvara=var(FSab(:,1));
        empmada=mad(FSab(:,1),1);
        
        if p==1
            empbiasb=(mean(FSab(:,2))-beta(1))^2;
            empvarb=var(FSab(:,2));
            empmadb=mad(FSab(:,2),1);
        else
            empvarb=0;
            empbiasb=0;
...
% questa section genera la Figura 5.15
sel1=(3:5);
% sel1=[9 14 18];
namx={'FS' 'LTS' 'LTSr' 'S' 'MM'};
sel1=([4 8 9 14]);
...
xl1=min(min((Xysto(:,1,sel1))));
xl2=max(max((Xysto(:,1,sel1))));


yl1=min(min((Xysto(:,2,sel1))));
yl2=max(max((Xysto(:,2,sel1))));
...
if prin==1
    % print to postscript
    print -depsc figs\sim1boxscarev1.eps;
end
...
subplot(2,2,1)
box('on')
biasAA=cumsum(abs([StoFS(:,ij) StoLT(:,ij) StoLTr(:,ij)  StoS(:,ij) StoMM(:,ij)]));
hold('on')
plot(lamx(:,1),biasAA(:,1),'LineWidth',LineWidth,'LineStyle','-', 'Color',col{5})
plot(lamx(:,1),biasAA(:,4),'LineWidth',LineWidth,'LineStyle','--','Marker','o', 'Color',col{1})
plot(lamx(:,1),biasAA(:,5),'LineWidth',LineWidth,'LineStyle','-','Marker','v', 'Color',col{2})
plot(lamx(:,1),biasAA(:,2),'LineWidth',LineWidth,'LineStyle','-.', 'Color',col{3})
...
hold('on')
box('on')
plot(lamx(:,1),varAA(:,1),'LineWidth',LineWidth,'LineStyle','-', 'Color',col{5})
plot(lamx(:,1),varAA(:,4),'LineWidth',LineWidth,'LineStyle','--','Marker','o', 'Color',col{1})
plot(lamx(:,1),varAA(:,5),'LineWidth',LineWidth,'LineStyle','-','Marker','v', 'Color',col{2})
plot(lamx(:,1),varAA(:,2),'LineWidth',LineWidth,'LineStyle','-.', 'Color',col{3})
...
hold('on')
box('on')
plot(lamx(:,1),biasBB(:,1),'LineWidth',LineWidth,'LineStyle','-', 'Color',col{5})
plot(lamx(:,1),biasBB(:,4),'LineWidth',LineWidth,'LineStyle','--','Marker','o', 'Color',col{1})
plot(lamx(:,1),biasBB(:,5),'LineWidth',LineWidth,'LineStyle','-','Marker','v', 'Color',col{2})
plot(lamx(:,1),biasBB(:,2),'LineWidth',LineWidth,'LineStyle','-.', 'Color',col{3})
...
hold('on')
box('on')
plot(lamx(:,1),varBB(:,1),'LineWidth',LineWidth,'LineStyle','-', 'Color',col{5})
plot(lamx(:,1),varBB(:,4),'LineWidth',LineWidth,'LineStyle','--','Marker', 'o','Color',col{1})
plot(lamx(:,1),varBB(:,5),'LineWidth',LineWidth,'LineStyle','-','Marker', 'v','Color',col{2})
plot(lamx(:,1),varBB(:,2),'LineWidth',LineWidth,'LineStyle','-.', 'Color',col{3})
