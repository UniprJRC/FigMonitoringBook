...
if prin==1
    % print to postscript
    print -depsc figs\MPGytraX.eps;
end
sgtitle('Figure A.77')
set(gcf,'Name','Figure A.77')
drawnow

%% Figures A.78 and A.79 are create by file Ex10_4Interactive.m
...
figure(pl_aug(end))
title('Figure A.80 (left-hand panel)')
set(gcf,"Name",'Figure A.80 (left-hand panel)')

pl_heat=findobj(0, 'type', 'figure','tag','pl_heatmap');
figure(pl_heat(end))
title('Figure A.80 (right-hand panel)')
set(gcf,"Name",'Figure A.80 (right-hand panel)')
...
xlabel(['g(' nameXycat{aa} ')'])
ylabel(['g(' nameXycat{bb} ')'])
zlabel('y')
hold('on');
...
if prin==1
    % print to postscript
    print -depsc figs\mpgXYZ.eps;
    print -depsc figs\mpgXYZ1.eps;
end

title('Figure A.83 (top or bottom panel depending on your rotation) ')
set(gcf,"Name",'Figure A.80')
