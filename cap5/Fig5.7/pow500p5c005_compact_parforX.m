...
for sj=shift
    ...
    for j=1:nsimul
       ...
       % end
        ...
        % if ~isnan(outMM.outliers)
            ...
            % Store number of true outliers
            PowermatMM(j,1)=length(trueoutMM);
        % else
        %    Powermat(j,2)=0;
        % end
        ...
        % if ~isnan(outLXS.outliers)
            ...
            % Store number of true outliers
            PowermatLTS(j,1)=length(trueoutLXS);
        % else
        %    Powermat(j,3)=0;
        % end
        ...
        % if ~isnan(outLXSr.outliers)
            ...
            % Store number of true outliers
            PowermatLTSr(j,1)=length(trueoutLXSr);
        % else
        %    Powermat(j,4)=0;
        % end
    ...
    outFS=FSR(ysimj,X,'nsamp',nsamp,'msg',0,'plots',0,'init',round(n/2));
       ...
       % if ~isnan(outFS.outliers)
            ...
            % Store number of true outliers
            PowermatFS(j,1)=length(trueoutFS);
%         else
%             Powermat(j,5)=0;
%         end
    ...
    avpowMM=mean(PowermatMM(:,1))/kk;
    avpowLXS=mean(PowermatLTS(:,1))/kk;
    avpowLXSr=mean(PowermatLTSr(:,1))/kk;
    avpowFS=mean(PowermatFS(:,1))/kk;
